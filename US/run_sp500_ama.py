"""S&P 500 AMA (KAMA) trend-following research — daily ^GSPC.

Tests Kaufman's Adaptive Moving Average (AMA / KAMA) crossover
on the S&P 500, where trends are expected to be more exploitable
than on FX pairs.

Strategy:
  - Long (1) when Close > AMA, Short (-1) when Close < AMA.
  - AMA adapts via the Efficiency Ratio: fast in trends, flat in chop.
...
_cached = '/tmp/bt_cache/^GSPC_19950101_20260530.csv'
if os.path.exists(_cached):
    df = pd.read_csv(_cached, index_col=0, parse_dates=True)
else:
    raise FileNotFoundError("Run US/scripts/fetch_sp500.py first")
...
class AMATrend(Strategy):
    ...
    Long (1) when Close > AMA, Short (-1) when Close < AMA.
    AMA is Kaufman's Adaptive Moving Average — speeds up in trends
    (high ER), flattens in chop (low ER).
    """
...
for sname, strat in [('AMA30', AMATrend(period=30, fast=2, slow=30)),
    ...
    fname = os.path.join(OUT, f'sp500_{sname.lower()}_folds.json')
    with open(fname, 'w') as f:
        json.dump(folds, f, default=str)
    generate_markdown_report(m, folds, '^GSPC', sname,
                             os.path.join(OUT, f'sp500_{sname.lower()}_report.md'))
    export_trade_log(r['trades'], os.path.join(OUT, f'sp500_{sname.lower()}_trades.csv'))
    plot_equity_curve(r['equity'], r['drawdown'], '^GSPC', sname,
                      os.path.join(CHARTS, f'sp500_{sname.lower()}_equity.png'))

with open(os.path.join(OUT, 'sp500_ama_results.json'), 'w') as f:
    json.dump(results, f, indent=2, default=str)

print("\n=== Summary ===")
print(json.dumps(results, indent=2, default=str))
print("\nAll S&P 500 reports saved to US/reports/ and US/charts/")
